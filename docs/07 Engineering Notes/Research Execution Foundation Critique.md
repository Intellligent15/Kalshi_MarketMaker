...
| The Python and C++ risk paths are both retained without a full trace-by-trace conformance suite. | Unnecessary transitional complexity | 4 | 3 | The compatibility path is useful, but two implementations can drift if only broad outcome tests exist. | Add fixture-driven parity tests for admission, rejection, partial fill, cancellation, and checkpoint states; then limit Python reference use to tests. |
