...
| Finding | Impact | Why it matters | Recommended handling |
| --- | ---: | --- | --- |
| The command and event journals are in memory only. | 5 | A process exit loses the audit trail, queued commands, terminal order history, and recovery point. | Design a versioned, checksummed write-ahead command journal and atomic event-batch store before calling recovery durable. |
| Event append is not transactional with book mutation. Sequence exhaustion or allocation failure after a successful book mutation can leave state without its complete event batch. | 5 | The simulator's central promise is that state and journal describe the same exchange history. A durable store makes this a correctness boundary. | Introduce prepared command execution: plan/reserve all IDs and event capacity, durably append an input/batch record, then commit matching and publication with defined recovery semantics. |
...
| Checkpoints are detached C++ values without format versioning, checksums, or compatibility guarantees. | 4 | Future field changes can make a persisted snapshot unreadable or silently misinterpreted. | Version snapshot schemas together with the durable journal; reject unknown versions rather than guessing. |
...
| 3 | ADR the durable write-ahead journal and atomic command/event-batch semantics. | 5 | Medium | Resolves the central state-versus-history correctness risk. |
| 4 | Implement versioned snapshots plus durable journal recovery and fault injection. | 5 | High | Required before credible historical replay, audit, or paper-trading claims. |
