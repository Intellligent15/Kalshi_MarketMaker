...
[[nodiscard]] Json ReadCanonicalJson(const Path& path) {
...
[[nodiscard]] core::Side SideField(const Json& value, const char* key,
                                   const std::string& location) {
...
[[nodiscard]] Path MemberPath(const Path& root, const std::string& name,
                              const std::string& location) {
...
[[nodiscard]] Operation ParseOperation(const Json& operation, const std::string& location) {
  ...
  Fail(location + ".operation", "is not supported by fixture V1");
}
...
[[nodiscard]] ExpectedState ParseState(const Json& state, const std::string& location) {
  ...
  for (std::size_t index = 0; index < state.at("pending_orders").size(); ++index) {
    ...
    if (!order.at("post_only").is_boolean() || !order.at("post_only").get<bool>()) {
      Fail(item_location + ".post_only", "must be true for fixture V1");
    }
...
std::filesystem::path FixtureRoot() {
  return Path(PMM_SOURCE_DIR) / "python" / "tests" / "fixtures" / "risk_conformance" / "v1";
}
...
std::vector<Fixture> LoadCorpus(const std::filesystem::path& root) {
  ...
  if (StringField(manifest, "schema", manifest_path.string()) != kManifestSchema ||
  ...
  CheckHash(payload_hash, manifest.at("payload").dump() + "\n",
  ...
  if (StringField(payload, "schema", manifest_path.string() + ".payload") != kManifestSchema ||
  ...
  for (std::size_t index = 0; index < payload.at("entries").size(); ++index) {
    ...
    fixtures.push_back(ParseFixture(fixture_document, trace_document, fixture_name, trace_name));
  }
